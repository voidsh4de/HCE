const engine = {
    ...
    window: {
        ...
        backgroundIMG: function(url) {
            document.getElementById("window").style.backgroundImage = `url(${url})`;
        }
    },
    //allows user input
    buttons: {
    ...
    items: {
        ...
        printItem: function(item, image="") {
            document.getElementById("items").innerHTML += `<div style="box-sizing: border-box; margin:0; padding:0; width: ${item._width}px; height: ${item._height}px; background-color: ${item.color}; position: relative; bottom: ${item._y + item._height}px; left: ${item._x}px; font-size: ${item._width / 2}px; background-image: url(${image}); display: inline-block; border-radius: ${item.radius}px; border: ${item.border}">${item.name}</div>`;
        },
